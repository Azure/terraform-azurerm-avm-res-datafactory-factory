...
- <a name="requirement_azurerm"></a> [azurerm](#requirement\_azurerm) (>= 3.87, < 5.0.0)
...
Description: Controls the API versions used for each `azapi_resource` declared by this module (TFFR6).  
...
Description: Retry configuration applied to every `azapi` resource managed by the module
...
Description: Default per-operation timeouts applied to every `azapi` resource managed by the  
